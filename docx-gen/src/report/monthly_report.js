                ...
                },
                {
                    ...
                    children: [
                        ...
                        paragraph({ // запасы жел руды в китай портах
                            children: [await oneChartText(FormChartUrl(new ChartUrl([28], StockId, GetNMonthRange(date, 5), 1, "bar", "month", "month", 1)))]
                        }),
                        paragraph({ //жрс 62 и 65
                            children: [await oneChartText(FormChartUrl(new ChartUrl([1,2], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        ...
                        paragraph({ // коксующийся уголь россия австралия
                            children: [await oneChartText(FormChartUrl(new ChartUrl([6,7], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await tableDouble(6, 7, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // коксующийся уголь россия австралия
                        paragraph({ // мет кокс
                            children: [await oneChartText(FormChartUrl(new ChartUrl([8], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        ...
                        paragraph({ // лом 3А
                            children: [await oneChartText(FormChartUrl(new ChartUrl([3], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await singleTableMinimax(3, GetNMonthRange(date, 9, true), 0, 1, "month", 1),// лом 3А
                        new docx.Paragraph({children: [new docx.PageBreak()]}),
                        h3(""),
                        await tableMaterialMinimax([70,71,72,73,74,75,76,77,78,79,80,81,82,83,84], GetFirstDaysOfCurrentAndPrevMonth(date), 0, 1, "month", 1),


                        h3("Чугун"),
                        paragraph({ // чугун фоб
                            children: [await oneChartText(FormChartUrl(new ChartUrl([5], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await singleTableMinimax(5, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // чугун фоб
                        new docx.Paragraph({children: [new docx.PageBreak()]}),
                        h3(""),
                        await tableMaterialMinimax([108, 109, 110], GetFirstDaysOfCurrentAndPrevMonth(date), 0, 1, "month", 1),
                        new docx.Paragraph({children: [new docx.PageBreak()]}),
                        ...
                        paragraph({ // заготовка сляб
                            children: [await oneChartText(FormChartUrl(new ChartUrl([9, 11], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await tableDouble(9, 11, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // коксующийся уголь россия австралия
                        await tableMaterialMinimax([85,86,87,88,89,90,91], GetFirstDaysOfCurrentAndPrevMonth(date), 0, 1, "month", 1),



                        h3("Сортовой прокат"),
                        paragraph({ // арматура FOB
                            children: [await oneChartText(FormChartUrl(new ChartUrl([10], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await singleTable(10, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // арматура FOB
                        h3Fake(" "),
                        paragraph({ // арматура A1
                            children: [await oneChartText(FormChartUrl(new ChartUrl([14], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await singleTable(14, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // арматура A1
                        new docx.Paragraph({children: [new docx.PageBreak()]}),
                        h3Fake(" "),
                        await tableMaterialMinimax([92, 93, 94], GetFirstDaysOfCurrentAndPrevMonth(date), 0, 1, "month", 1),


                        h3("Плоский прокат"),
                        paragraph({ // гк хк FOB
                            children: [await oneChartText(FormChartUrl(new ChartUrl([12, 13], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await tableDouble(12, 13, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // гк хк FOB
                        new docx.Paragraph({children: [new docx.PageBreak()]}),
                        h3(""),
                        paragraph({ // гк хк EXW
                            children: [await oneChartText(FormChartUrl(new ChartUrl([15, 16], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await tableDouble(15, 16, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // гк хк EXW
                        await tableMaterialMinimax([95,96,97,98,99,100,101,102,103,104,105,106,107], GetFirstDaysOfCurrentAndPrevMonth(date), 0, 1, "month", 1),
                        new docx.Paragraph({children: [new docx.PageBreak()]}),



                        h3("Рынок ферросплавов и руд"),
                        await tableMaterialGrouped([17,18,19,20,21,22,23], [date, lastDayOfMonth], [0, 5], ["Ферросплавы (DDP Европа)", "Руда (CIF Китай)"], "month", 1),



                        h3("Ферромарганец и силиконмарганец"),
                        paragraph({ // FeMn SiMn
                            children: [await oneChartText(FormChartUrl(new ChartUrl([17, 19], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await tableDouble(17, 19, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // гк хк EXW



                        h3("Ферросилиций"),
                        paragraph({ // FeSi
                            children: [await oneChartText(FormChartUrl(new ChartUrl([18], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await singleTable(18, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // арматура A1


                        h3("Феррохром"),
                        paragraph({ // HC LC FeCr
                            children: [await oneChartText(FormChartUrl(new ChartUrl([20, 21], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await tableDouble(20, 21, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // гк хк EXW
                        new docx.Paragraph({children: [new docx.PageBreak()]}),
                        ...
                        paragraph({ // mn руда запасы в китае
                            children: [await oneChartText(FormChartUrl(new ChartUrl([26], StockId, GetNMonthRange(date, 5), 1, "bar", "month", "month", 1)))]
                        }),
                        paragraph({ // mn руда
                            children: [await oneChartText(FormChartUrl(new ChartUrl([22], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await singleTable(22, MedPriceId, GetNMonthRange(date, 9, true), 2, 1, "month", 1), // арматура A1



                        h3("Хромовая руда"),
                        paragraph({ // cr руда запасы в китае
                            children: [await oneChartText(FormChartUrl(new ChartUrl([27], StockId, GetNMonthRange(date, 5), 1, "bar", "month", "month", 1)))]
                        }),
                        paragraph({ // cr руда
                            children: [await oneChartText(FormChartUrl(new ChartUrl([23], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await singleTable(23, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1), // арматура A1



                        h2("Рынок графитированых электродов"),
                        paragraph({ // гэ 450 600 мм
                            children: [await oneChartText(FormChartUrl(new ChartUrl([24, 25], MedPriceId, GetNMonthRange(date, 9), 1, "line", "month", "month", 1)))]
                        }),
                        await tableDouble(24, 25, MedPriceId, GetNMonthRange(date, 9, true), 0, 1, "month", 1),
                    ],
                },
            ],
        });
